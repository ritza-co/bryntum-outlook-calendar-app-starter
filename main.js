...
import { getEvents, createEvent, getAllEvents, updateEvent, deleteEvent } from './graph.js';
...
async function updateMicrosoft(event) {
    ...
    // event is deleted
        ...
        const microEvents = await getAllEvents();
        const eventName = event.records[0].data.name;
        for (let i = 0; i < microEvents.value.length; i++) {
...
async function displayUI() {
    ...
    events.value.forEach((event) => {
        // Convert Outlook date format to ISO 8601 compatible string
        const startDate = event.start.dateTime.split('.')[0];  // Remove fractional seconds
